  ...
  const handleSearch = async () => {
    ...
    try {
      const res = await fetch(
        `http://localhost:5000/api/salons?categorie=institut&ville=${encodeURIComponent(ville)}`
      )
    ...
    <div className="bg-white">
      ...
      >
        <div className="absolute inset-0 bg-opacity-50 flex items-center justify-center text-white text-center">
          <div>
            <h1 className="text-4xl md:text-5xl font-bold mb-4">Trouvez votreInstitut de Beauté</h1>
            <p className="text-lg">
          Réserver en ligne un RDV en Institut de Beauté
            </p>
          </div>
        </div>
      </section>
